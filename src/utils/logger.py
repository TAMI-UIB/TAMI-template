...
class Logger():
    def __init__(self, dataset, model_name, nickname):
        day = datetime.now().strftime("%Y-%m-%d")
        self.model_name = model_name
        self.nickname = nickname
        self.dir_path = f"{os.environ["PROJECT_ROOT"]}/logs/{dataset}/{model_name}/{day}/{nickname}" if nickname is not None else f"{os.environ["PROJECT_ROOT"]}/logs/{dataset}/{model_name}/{day}"
    ...
    def save_checkpoints(self, model, epoch, validation_loss):
        ...
        torch.save(ckpt, f"{self.dir_path}/checkpoints/last.cpkt")
        if validation_loss < self.best_loss:
            self.best_loss = validation_loss
            torch.save(ckpt, f"{self.dir_path}/checkpoints/best.cpkt")
